...
namespace FinalSE.UI
{
	public partial class frmProduct : Form
	{
		...
		private void dataGridViewProducts_Click(object sender, EventArgs e)
		{
			...
			try
			{
				string exeFile = (new System.Uri(Assembly.GetEntryAssembly().CodeBase)).AbsolutePath.Substring(0, 50);
				string absolutePath = Path.GetDirectoryName(exeFile);
				string fileName = absolutePath + "\\img\\" + richtxtPhoto.Text;
		...
		private void btnBrowse_Click(object sender, EventArgs e)
		{
			...
			try
			{
				if (open.ShowDialog() == DialogResult.OK)
				{
					...
					richtxtPhoto.Text = open.FileName.Substring(49);
					MessageBox.Show(richtxtPhoto.Text);
